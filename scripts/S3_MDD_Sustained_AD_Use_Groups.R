...
#-- Phenotypes including lifetime MDD
pheno <- read.csv("/QRISdata/Q7280/pharmacogenomics/phenotypes/survey_phenotypes.csv") %>% 
  select(STUDYID, MDD)
...
durations <- c(360, 600)
#-- Vector of adherence
adherence_filters <- c("All", "Adherent")

for (duration in durations) {
  
  for (adherence in adherence_filters) {
    
    #-- Load treatment groups
    if (adherence == "All") {
      groups <- read.csv(paste0("/scratch/user/uqawal15/TEMP_TreatmentGroups_", duration, "days.csv"), header=TRUE)
    } else {
      groups <- read.csv(paste0("/scratch/user/uqawal15/TEMP_TreatmentGroups_", duration, "days_Adherent.csv"), header=TRUE)
    }
    
    #-- Filter for those with MDD
    groups <- groups %>%
      left_join(pheno, by = c("ParticipantID" = "STUDYID")) %>%
      filter(MDD == 1)
    ...
    #-- Filter treatment groups for those with BIP and L (BIP+L)
    groups_BIP_L <- groups %>%
    ...
    #-- Filter treatment groups for those without L
    ...
    #DrugName_keep <- final_mapped %>%
    #  group_by(DrugName) %>%
    #  summarise(
    #    Count = n()
    #  ) %>%
    #  filter(Count >=50 | DrugName %in% c("BIP-L", "BIP+L", "Combination", "Various"))
    
    #DrugClass_keep <- final_mapped %>%
    #  group_by(DrugClass) %>%
    #  summarise(
    #    Count = n()
    #  ) %>%
    #  filter(Count >=50 | DrugClass %in% c("BIP-L", "BIP+L", "Combination", "Various"))
    
    #final_mapped_other <- final_mapped %>%
    #  mutate(DrugName = case_when(
    #    DrugName %in% DrugName_keep$DrugName ~ DrugName,
    #    !DrugName %in% DrugName_keep$DrugName ~ "Other",
    #    TRUE ~ NA_character_),
    #    DrugClass = case_when(
    #      DrugClass %in% DrugClass_keep$DrugClass ~ DrugClass,
    #      !DrugClass %in% DrugClass_keep$DrugClass ~ "Other",
    #      TRUE ~ NA_character_
    #    ))
    ...
    if (adherence == "All") {
      ...
      write.csv(final_mapped, paste0("/scratch/user/uqawal15/Final_Treatmentgroups_", duration, "days_Adherent.csv"), row.names=FALSE)
    }
