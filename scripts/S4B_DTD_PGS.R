...
# -- Genetic data setup
pgslist <- system('find /QRISdata/Q7280/pharmacogenomics/pgs -name "*agds_sbrc_gctb_plink2.sscore" -type f -exec ls {} \\;', intern = TRUE)
pgs_codes <- c("ADHD_01", "BIP_LOO", "MDD_LOO", "Neuroticism_01")
selected_pgs <- pgslist[grep(paste(pgs_codes, collapse = "|"), pgslist)]
...
for (j in 1:length(selected_pgs)) {
  ...
  pgs_s <- pgs_e %>%
    mutate(SCORE1_SUM = as.numeric(V6),
           std_pgs = scale(V6)[,1]) %>%
    select(V2, SCORE1_SUM, std_pgs) %>%
    setNames(c("V2", paste0("SUM_", trait), trait))
  
  dat <- inner_join(dat, pgs_s, by = c("IID" = "V2"))

}

# ranked PGS by square-root of heritability
dat_multiPGS <- dat %>%
  mutate(
    multiPGS = ADHD_01 + BIP_LOO + MDD_LOO + Neuroticism_01,
    multiPGS_ranked = (0.3268*SUM_ADHD_01 + 0.14001*SUM_BIP_LOO + 0.16933*SUM_MDD_LOO + 0.34347*SUM_Neuroticism_01),
    multiPGS_ranked_sd = scale(0.3268*SUM_ADHD_01 + 0.14001*SUM_BIP_LOO + 0.16933*SUM_MDD_LOO + 0.34347*SUM_Neuroticism_01)
  )

# Density plot
p <- ggplot(dat_multiPGS, aes(x = multiPGS_ranked)) +
  geom_density(fill = "steelblue", alpha = 0.5) +
  labs(x = "Weighted PGS", y = "Density")
p2 <- ggplot(dat_multiPGS, aes(x = multiPGS_ranked_sd)) +
  geom_density(fill = "steelblue", alpha = 0.5) +
  labs(x = "Weighted PGS (scaled)", y = "Density")

png(
  filename = file.path("/scratch/user/uqawal15", "Weighted_PGS_Distribution.png"),
...
png(
  filename = file.path("/scratch/user/uqawal15", "Scaled_Weighted_PGS_Distribution.png"),
  width = 150, height = 150, units = 'mm',
  bg = "white", res = 600, type = c("cairo")
)
print(p2)
...
#-- Join with PCs
pharma_link <- inner_join(link, pharma_full, by = "ParticipantID")
pgs_pharma <- inner_join(pharma_link, dat_multiPGS, by = c("IID"))
pgs_pharma_pcs <- left_join(pgs_pharma, pcs, by = "IID")

#======= Analysis 1: Total Prescription Days
total_lm <- lm(TotalPrescriptionDays ~ AGE + SEX + PC1 + PC2 + PC3 + multiPGS_ranked_sd, data = pgs_pharma_pcs) 
lm_summary <- tidy(total_lm) %>% rename(Term = term)

lm_dat <- tibble(
  Dependent = "Cumulative Prescription Dispense (days)",
  PGS = "weighted_multiPGS",
...
#====== Analysis 2: Number of unique Antidepressants
numAD_lm <- lm(num_ATC ~ AGE + SEX + PC1 + PC2 + PC3 + multiPGS_ranked_sd, data = pgs_pharma_pcs) 
lm_summary <- tidy(numAD_lm) %>% rename(Term = term)

lm_dat2 <- tibble(
  Dependent = "Medication Diversity",
  PGS = "weighted_multiPGS",
  Analysis = "Full_Sample"
) %>%
  bind_cols(lm_summary)
...
#====== Analysis 3: Number of unique Antidepressant Classes
numClass_lm <- lm(num_class ~ AGE + SEX + PC1 + PC2 + PC3 + multiPGS_ranked_sd, data = pgs_pharma_pcs) 
lm_summary <- tidy(numClass_lm) %>% rename(Term = term)

lm_dat3 <- tibble(
  Dependent = "Class Diversity",
  PGS = "weighted_multiPGS",
  Analysis = "Full_Sample"
) %>%
  bind_cols(lm_summary)
...
results_final <- bind_rows(lm_dat, lm_dat2, lm_dat3) %>%
  mutate(
    across(c(p.value), ~ format(signif(.x, 2), scientific = TRUE)),
    std.error = if_else(Dependent %in% c("Medication Diversity", "Class Diversity"),
                        round(std.error, 4), round(std.error, 2)),
    estimate = if_else(Dependent %in% c("Medication Diversity", "Class Diversity"),
                       round(estimate, 4), round(estimate, 1)),
...
# Reorder columns for clarity
results_final <- results_final %>%
  select(Analysis,Dependent, PGS, Term, estimate, std.error, statistic, p.value, 
         everything()) %>%
  group_by(Analysis, Dependent, PGS) %>%
  mutate(
    Analysis = if_else(Analysis != lag(Analysis, default = ""), Analysis, NA_character_),
...
# Save to Excel
wb <- loadWorkbook(file.path("/scratch/user/uqawal15", "All_Results.xlsx"))
removeWorksheet(wb, "Table26")
addWorksheet(wb, "Table26")
writeData(wb, "Table26", results_final)
saveWorkbook(wb, file.path(output_dir, "All_Results.xlsx"), overwrite = TRUE)
