...
suppressPackageStartupMessages({
  ...
  library(parallel)
  library(openxlsx)
  library(readr)  # Added for faster CSV reading
})
...
for (duration in duration_list) {
  ...
  for (i in seq_along(exposome_binary)) {
    ...
    if (column_name %in% c("WELLAD", "STOPAD")) {
      excluded_drugs <- c("BIP-L", "BIP+L", "Control", "Combination")
      tab <- tab[!tab$DrugName %in% excluded_drugs, ]
      
      excluded_classes <- c("BIP+L", "Various")
...
process_results_for_excel <- function(df, data_type) {
  ...
  total_n <- df %>%
    filter(Reference == ref_filter_val, Threshold == 360) %>%
    rowwise() %>%
  ...
  results <- df %>%
    ...
    group_by(Threshold, Reference, Dependent) %>%
    mutate(
